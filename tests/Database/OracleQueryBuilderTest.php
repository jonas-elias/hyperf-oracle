...
class OracleQueryBuilderTest extends TestCase
{
    ...
    public function testUnions()
    {
        ...
        $this->assertSame('(select * from "USERS" where "ID" = ? union select * from "USERS" where "ID" = ?) union select * from "USERS" where "ID" = ?',
            $builder->toSql());
        $this->assertEquals([0 => 1, 1 => 2], $builder->getBindings());
    }
    ...
    public function testMultipleUnions()
    {
        ...
        $this->assertEquals('(select * from "USERS" where "ID" = ? union select * from "USERS" where "ID" = ? union select * from "USERS" where "ID" = ?) union select * from "USERS" where "ID" = ? union select * from "USERS" where "ID" = ?',
            $builder->toSql());
        $this->assertEquals([0 => 1, 1 => 2, 2 => 3], $builder->getBindings());
    }
    ...
    public function testMultipleUnionAlls()
    {
        ...
        $this->assertEquals('(select * from "USERS" where "ID" = ? union all select * from "USERS" where "ID" = ? union all select * from "USERS" where "ID" = ?) union all select * from "USERS" where "ID" = ? union all select * from "USERS" where "ID" = ?',
            $builder->toSql());
        $this->assertEquals([0 => 1, 1 => 2, 2 => 3], $builder->getBindings());
    }
    ...
    public function testUnionOrderBys()
    {
        ...
        $this->assertEquals('(select * from "USERS" where "ID" = ? union select * from "USERS" where "ID" = ? order by "ID" desc) union select * from "USERS" where "ID" = ? order by "ID" desc',
            $builder->toSql());
        $this->assertEquals([0 => 1, 1 => 2], $builder->getBindings());
    }
    ...
    public function testUnionLimitsAndOffsets()
    {
        ...
        $this->assertSame('(select * from "USERS" union select * from "DOGS") union select * from "DOGS"  ', $builder->toSql());

        $builder = $this->getBuilder();
        $expectedSql = '(select "A" from "T1" where "A" = ? and "B" = ? union select "A" from "T2" where "A" = ? and "B" = ? order by "A" asc) union select "A" from "T2" where "A" = ? and "B" = ? order by "A" asc ';
    ...
    public function testUnionWithJoin()
    {
        ...
        $this->assertSame('(select * from "USERS" union select * from "DOGS" inner join "BREEDS" on "DOGS"."BREED_ID" = "BREEDS"."ID" and "BREEDS"."IS_NATIVE" = ?) union select * from "DOGS" inner join "BREEDS" on "DOGS"."BREED_ID" = "BREEDS"."ID" and "BREEDS"."IS_NATIVE" = ?', $builder->toSql());
        $this->assertEquals([0 => 1], $builder->getBindings());
    }
    ...
    public function testGroupBys()
    {
        ...
        $builder = $this->getBuilder();
        $builder->havingRaw('?', ['havingRawBinding'])->whereRaw('?', ['whereRawBinding']);
        $this->assertEquals(['whereRawBinding', 'havingRawBinding'], $builder->getBindings());
    }
